...
#QEMU = /ust/local/bin/qemu-system-i386
...
# GCC with -O1 or greater, gcc will omit the framepointer for those functions
# whose framesize is static. It is still possible to do backtrace, by 
# knowing the framesize of the function and winding up the stack by 
# substracting the framesize. It is possible to disable this behaviour of gcc
# by specifying -fno-omit-frame-pointer(gcc will reuse ebp though).
# For time being, we disable optimization with -O0
CFLAGS := $(CFLAGS) $(DEFS) $(LABDEFS) -O0 -fno-omit-frame-pointer -fno-builtin -I$(TOP) -MD 
